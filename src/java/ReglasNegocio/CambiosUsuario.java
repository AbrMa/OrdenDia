 ...
 * @author Carmen
 */
@WebServlet(name = "CambiooUsuario", urlPatterns = {"/CambiooUsuario"})
public class CambiosUsuario extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
             ...
             if(usuario!=null && pass !=null &&  direc!=null  && correo!=null)
            {
                ...
                {
                    sql.conectar();
                    ResultSet rsUsr = sql.consulta("call sp_CambioUsuario('"+usuario+"','"+direc+"','"+correo+"','"+pass+"');");
                      
                    while(rsUsr.next())
                    {
                        ResultSet rsUsr2 = sql.consulta("call st_consultasUsuario('" + correo + "','" + pass + "');");
                         String direct = rsUsr2.getString("Direccion");
                         String nom = rsUsr2.getString("Nombre");
                         HttpSession op = request.getSession();
                           op.putValue("Direc", direct);
