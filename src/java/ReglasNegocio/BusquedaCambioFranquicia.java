 ...
 * @author Alejandro
 */
@WebServlet(name = "BusquedaCambioFranquicia", urlPatterns = {"/BusquedaCambioFranquicia"})
public class BusquedaCambioFranquicia extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            String FRAN = request.getParameter("franquicia");
            if (FRAN != null) {
                BD.cDatos sql = new BD.cDatos();
                try {
                    sql.conectar();
                   ResultSet rsUsr = sql.consulta("call sp_ConsultaFranquicia3('" +FRAN + "');");
                    ...
                    + "        <!--Formulario-->\n"
                    + "        <div class=\"container\">\n"
                    + "            <center><h3>Cambiar Franquicia</h3></center>\n"
                    + "<h3>Cambiar Franquicia</h3>\n"          
                    + "            <hr>\n"
                    + "        </div>  \n"
                    + "        <div class=\"container\">\n"
                    + "            <form class=\"form-horizontal\" role=\"form\" method=\"post\" name=\"CambioFranq\" action=\"CambiaFranquicia.jsp\">\n"
                    + "                <div class=\"form-group\">\n"
                    + "                        <label class=\"control-label col-sm-2\" for=\"franchise\">Nombre Franquicia</label>\n"
                    + "                    <div class=\"col-sm-10\">\n"
                    + "                        <input type=\"text\" class=\"form-control\" id=\"franchise\" name=\"franquicia\" value='"+Nombre+"' required>\n"
                    + "                    </div>\n"
                    + "                </div>\n"
                    + "                <div class=\"form-group\">\n"
